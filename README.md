...
selection** controls it. v2 adds three brain types, a hybrid selection regime,
and a parallel, restartable runner for serious compute.

## What's new vs v1
- **Three controllers** behind one interface: `feedforward` MLP, `recurrent`
  Elman RNN (memory), and a custom **NEAT** (evolving topology + recurrence).
...
python run_experiments.py \
    --controllers feedforward,recurrent,neat \
...
| File | Role |
|------|------|
| `evolved_comm/config.py` | One `Config` dataclass = one fully-specified run (world, controller, GA, regime, NEAT, checkpoint). |
...
| `evolved_comm/neat.py` | Custom NEAT: genome/innovations, mutation, crossover, speciation, batched recurrent network eval, and the speciated reproduction step. |
| `evolved_comm/evolution.py` | Brain-agnostic core: group assembly + colony/individual/hybrid fitness, the fixed-topology GA, MI probe, checkpointing, and the `evolve()` dispatch. |
...
| `make_figures.py` | Adapts to whatever runs exist: learning + emergence per controller, value bar, hybrid alpha-sweep, NEAT complexity, signal map, `stats.json`. |
